...
df_incl=read.delim('./input/included_studies.csv',sep=',')
df_data=read.delim('./input/extracted_data.csv',sep=',')

# with specialty
t=df_incl%>%select(c(X.source..id,specialty))
df_data_spec=join(df_data, t, by='X.source..id', type="left")
#df_data_spec=df_data_spec%>%select(-29)

df_search=read.delim('./input/search.csv',sep=',')
df_summary=read.delim('./input/summary_data.csv', sep=',')
...
df_corr_au=read.delim('./input/characteristics_of_reviews.csv',sep=',')
# adds whether the corresponding author was a surgeon or not
df_summary=left_join(df_summary,df_corr_au, by='X.source..id')
df_summary=join(df_summary, t, by='X.source..id', type="left")
...
df_re_anal=read.csv('./input/re_meta_manual.csv')[1:105,]
...
df_rct=read.csv('./input/journal_RCT.csv')
